...
PH_cognate <- compute_network_distance_df(
  PH_cognate,
  nodes,
  network_edges,
  land_sf,
...
PH_cognate |>
  select(language_id, language, glottocode, author,
         latitude, longitude,
...
# row_attrs is a constant TRUE, not PH_cognate$using_network: §5 always keeps
...
connector_sf <- bind_rows(
  geom_rows(PH_cognate$land_geom, TRUE,  "connector", rep(TRUE, nrow(PH_cognate))),
  geom_rows(PH_cognate$sea_geom,  FALSE, "connector", rep(TRUE, nrow(PH_cognate)))
)
...
plot_network <- function(full_tree_sf, arrow_connectors,
  ...
  ggplot() +
    ...
    labs(title = "Cognate Historical Routes Waypoint System")
}

print(plot_network(full_tree_sf, arrow_connectors,
                   PH_cognate, refdf1 = MANILA))


# ── 10. Arrow-only plot (main tree + connector arrows, all black) ───────────
# Built as a transparent overlay layer for the loanword point map in [7].
arrow_plot <- ggplot() +
  geom_sf(data = full_tree_sf |> filter(source == "main"),

# =============================================================================
# [4.3] Cognate Analysis — regional PVR-MEM
#
...
#          figures/cognate/regression/cognate_regional_pvr_mem_<region>.png
...
# ── 1. Analysis frame carrying region (same construction as [4.1]) ─────────
df_all <- COGNATE_final |>
  ...
  as.data.frame()
df_all <- df_all[match(tree_pruned$tip.label, df_all$glottocode), ]
...
stopifnot(
  ...
  "Analysis frame is not in tip.label order — PVR matches by position." =
    !anyNA(df_all$glottocode) && identical(df_all$glottocode, tree_pruned$tip.label),
  "Some languages have no region — rerun [0]_Phylogenetic_Tree.R." = !anyNA(df_all$region)
)
...
for (rg in REGIONS) {
  ...
  N  <- nrow(df)
  row <- tibble(region = rg, n = N, status = NA_character_,
                n_evec_phylo = NA_integer_, threshold_km = NA_real_,
                moran_I = NA_real_, moran_perm_p = NA_real_, n_perm = N_PERM,
  ...
  tree_rg <- drop.tip(tree_pruned, setdiff(tree_pruned$tip.label, df$glottocode))
  df <- df[match(tree_rg$tip.label, df$glottocode), ]
  stopifnot(
    "Regional frame is not in regional tip.label order." =
      identical(df$glottocode, tree_rg$tip.label),
  ...
  scores[[rg]] <- tibble(glottocode = df$glottocode, language = df$language,
                         longitude = df$longitude, latitude = df$latitude,
                         sPC1 = fitted(mem_fit), region = rg)
...
panel <- function(rg) {
  ...
  st <- COGNATE_regional_results |> filter(region == rg)
  ggplot() +
...
# are separate.
run_regions <- COGNATE_regional_results |> filter(status == "ok") |> pull(region)
panels <- lapply(run_regions, panel)
names(panels) <- run_regions

for (rg in run_regions) {
  print(panels[[rg]])
  ggsave(here("figures", "cognate", "regression",
              paste0("cognate_regional_pvr_mem_", tolower(rg), ".png")),
         panels[[rg]], width = 5.5, height = 6, units = "in", dpi = 300)
}

skipped <- COGNATE_regional_results |> filter(status != "ok")
if (nrow(skipped) > 0) {

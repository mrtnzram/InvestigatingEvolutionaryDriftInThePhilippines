...
#
# Fits three models of Spanish-phoneme cosine similarity vs. terrain-penalized
...
PHONEME_pgls_results <- bind_rows(
  tidy_pgls(pgls_ols, "OLS (lambda fixed ~0)"),
  tidy_pgls(pgls_fit, "ML (lambda estimated)")
)
print(PHONEME_pgls_results, n = Inf)
write.csv(PHONEME_pgls_results,
          file = here("data", "PHONEME_pgls_results.csv"), row.names = FALSE)
...
ggsave(here("figures", "phoneme", "regression", "phoneme_linear_model.png"),
       p_lin, width = 7, height = 4.5, units = "in", dpi = 300)
ggsave(here("figures", "phoneme", "regression", "phoneme_exponential_model.png"),
       p_exp, width = 7, height = 4.5, units = "in", dpi = 300)
ggsave(here("figures", "phoneme", "regression", "phoneme_spline_model.png"),
...
report <- tibble(
  ...
  arrange(WAIC)
print(report)
write.csv(report, file = here("data", "PHONEME_waic_compare.csv"), row.names = FALSE)

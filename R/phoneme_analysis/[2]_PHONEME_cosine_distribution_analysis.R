...
qq_plot <- ggplot(null_sel_long, aes(sample = similarity)) +
  ...
  labs(title = "Phoneme individual null distribution QQ plots",
...
pick_influenced <- function(inf_col, obs_col, influence_name) {
  PHONEME_cossim |>
    ...
    arrange(observed) |>
    slice(round(quantile(seq_len(n()), probs = c(0.25, 0.5, 0.75))))  # low / mid / high
}
...
marker_df <- null_ridge_langs |>
  left_join(PHONEME_cossim |> select(language, Spanish = cossim_span,
...
observed <- list(
  Spanish  = setNames(PHONEME_cossim$cossim_span, PHONEME_cossim$language),
  Japanese = setNames(PHONEME_cossim$cossim_jap,  PHONEME_cossim$language),
  English  = setNames(PHONEME_cossim$cossim_eng,  PHONEME_cossim$language)
)
...
venn_setlabs <- tibble(
  x   = c(0.00, -1.20, 1.20),
  y   = c(1.62, -0.95, -0.95),
  lab = c(paste0("Spanish (n=",  sum(PHONEME_cossim$span_influenced), ")"),
          paste0("Japanese (n=", sum(PHONEME_cossim$jap_influenced),  ")"),
          paste0("English (n=",  sum(PHONEME_cossim$eng_influenced),  ")"))
)
...
# Fold per-language significance into PHONEME_cossim (sig_<baseline>).
...
PHONEME_cossim_marked <- PHONEME_cossim |>

...
#          geography the phoneme and grammar analyses use.
# ──────────────────────────────────────────────────────────────────────────────
...
# ── 0. Load data ─────────────────────────────────────────────────────────────
PH_genetic <- read.csv(here("data", "GENETIC_admixture.csv"))
...
# Used for BOTH network edges and population→node connectors, so the land/sea
...
# ── 5. Per-population: terrain-penalized distance to the network ─────────────
# geodist_H1_span : penalized cost from the population point to its nearest
...
compute_network_distance_df <- function(df, nodes, edges, land_sf,
  ...
  df_geom |>
    ...
    # on distance — a plotting-only choice, so every population visibly joins the
...
PH_genetic <- compute_network_distance_df(
  PH_genetic,
  nodes,
  network_edges,
  land_sf,
...
PH_genetic |>
  select(any_of(c("population", "nearest_node", "geodist_H1_span",
                  "land_len", "sea_len", "crosses_land"))) |>
  arrange(geodist_H1_span) |>
  print(n = 20)

PH_genetic |>
  select(any_of(c("population", "nearest_node", "geodist_H2_span",
...
se_q <- quantile(PH_genetic$span_se, c(SE_Q_LO, SE_Q_HI), na.rm = TRUE)

PH_genetic <- PH_genetic |>
  mutate(
    span_se_capped = pmin(pmax(span_se, se_q[[1]]), se_q[[2]]),
    span_w         = 1 / span_se_capped^2,
    span_w_capped  = !is.na(span_se) & (span_se < se_q[[1]] | span_se > se_q[[2]])
  )

message(sprintf("span_se caps: [%.4f, %.4f]; weights %.1f-%.1f; %d populations capped.",
                se_q[[1]], se_q[[2]],
                min(PH_genetic$span_w, na.rm = TRUE), max(PH_genetic$span_w, na.rm = TRUE),
                sum(PH_genetic$span_w_capped, na.rm = TRUE)))


# Flat columns only (geometry list-columns dropped) for [4] and [7] to consume.
PH_genetic |>
  select(population, model_region, island_group, region, province, precision_tier,
         latitude, longitude, model,
         ends_with("_admx"), ends_with("_se"),
         effblocks, qpadm_pval,
         starts_with("malder_"),
         span_se_capped, span_w, span_w_capped,
         nearest_node, geodist_H1_span, geodist_H2_span, using_network) |>
  write.csv(file = here("data", "GENETIC_final.csv"), row.names = FALSE)


# ── 7. Assemble full_tree_sf (main edges + per-population connectors) ────────
# row_attrs is a constant TRUE, not PH_genetic$using_network: §5 always keeps
...
geom_rows <- function(geom_list, crosses_land_value, source_value,
  ...
  # rep(times = n_per_row) keeps per-population attrs aligned with the
...
connector_sf <- bind_rows(
  geom_rows(PH_genetic$land_geom, TRUE,  "connector", rep(TRUE, nrow(PH_genetic))),
  geom_rows(PH_genetic$sea_geom,  FALSE, "connector", rep(TRUE, nrow(PH_genetic)))
)
...
# ── 9. Overview plot: network + population points + directional arrows ──────
# Main network edges draw plain: they are bidirectional, so only the directional
# population -> node connectors carry arrowheads.
plot_network <- function(full_tree_sf, arrow_connectors,
  ...
  ggplot() +
    ...
    labs(title = "Genetic Historical Routes Waypoint System")
}

print(plot_network(full_tree_sf, arrow_connectors,
                   PH_genetic, refdf1 = MANILA))


# ── 10. Arrow-only plot (main tree + connector arrows, all black) ───────────
# Built as a transparent overlay layer for the admixture point map in [7].
arrow_plot <- ggplot() +
  geom_sf(data = full_tree_sf |> filter(source == "main"),
...
print(arrow_plot)
saveRDS(arrow_plot, file = here("data", "genetic_waypoint_plot.rds"))
